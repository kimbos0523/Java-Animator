...
public class AnimationReader {
  ...
  private static <Doc> void readLayer(Scanner s, AnimationBuilder<Doc> builder) {
    ...
    if (s.hasNext()) {
      ...
      throw new IllegalStateException("Layer: Expected a shape name, but no more input available");
    }
    int val = getInt(s, "Layer", "Layer");
    builder.setLayer(name, val);
  }
  ...
  private static <Doc> void readMotion(Scanner s, AnimationBuilder<Doc> builder) {
    String[] fieldNames = new String[]{
      ...
      "final red value", "final green value", "final blue value", "initial degree", "final degree"
    };
    int[] vals = new int[18];
    String name;
    if (s.hasNext()) {
