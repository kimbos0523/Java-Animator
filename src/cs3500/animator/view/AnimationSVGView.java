...
public class AnimationSVGView implements AnimationView {
  ...
  private void attributeRec(AnimationShape s1, AnimationShape s2, Appendable ap,
    ...
    if (s1.getShapeColor().getR() != s2.getShapeColor().getR()
        || s1.getShapeColor().getG() != s2.getShapeColor().getG()
        || s1.getShapeColor().getB() != s2.getShapeColor().getB()) {
      String result = "\t<animate" + makeTag("attributeType", "xml")
          + makeTag("begin", begin) + makeTag("dur", dur)
          + makeTag("attributeName", "fill")
          + makeTag("from", s1.getShapeColor().svgString())
          + makeTag("to", s2.getShapeColor().svgString()) + makeTag("fill", "freeze")
          + " />\n";
      ap.append(result);
    }
    if (s1.getDegree() != s2.getDegree()) {
      String result = "\t<animateTransform" + makeTag("attributeType", "xml")
          + makeTag("begin", begin) + makeTag("dur", dur)
          + makeTag("attributeName", "transform")
          + makeTag("type", "rotate")
          + makeTag("from", s1.getDegree(), (s1.getPosition().getX() + (s1.getWidth() / 2)),
          (s1.getPosition().getY() + (s1.getHeight() / 2)))
          + makeTag("to", s2.getDegree(), (s2.getPosition().getX() + (s2.getWidth() / 2)),
          (s2.getPosition().getY() + (s2.getHeight() / 2)))
          + makeTag("repeatCount", 1)
          + " />\n";
      ap.append(result);
    }
    if (s1.equals(s2)) {
  ...
  private String makeTag(String tag, int value, int v2, int v3) {
    return String.format(" %s=\"%d %d %d\"", tag, value, v2, v3);
  }
